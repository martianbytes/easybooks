...
class BookAdmin(admin.ModelAdmin):
    list_display = ['title', 'seller', 'asking_price', 'condition', 'status', 'created_at']
    ...
    fieldsets = (
        ('Core Information', {
            'fields': ('title', 'slug', 'subtitle', 'authors', 'publisher', 'published_year', 'edition', 'isbn')
        }),
        ('Details', {
            'fields': ('language', 'num_pages', 'category', 'description')
        }),
        ('Condition', {
            'fields': ('condition', 'condition_notes')
        }),
        ('Pricing', {
            'fields': ('original_price', 'asking_price', 'is_negotiable')
        }),
        ('Listing', {
            'fields': ('seller', 'status', 'city', 'is_active')
        }),
        ('Metadata', {
