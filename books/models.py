...
class Book(models.Model):
    ...
    subtitle = models.CharField(max_length=255, blank=True) # Optional extra title line.
...
class BookImage(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.pk:  # Only for new instances
            max_order = BookImage.objects.filter(book=self.book).aggregate(models.Max('order'))['order__max']
